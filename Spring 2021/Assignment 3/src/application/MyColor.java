...
enum MyColor {
	...
	CANVAS_WHITE(244,244,244,244),
	WHITE(255,255,255,255);
	
	static MyColor [] randomBlackList = {LIGHT_SALMON, PALE_GOLDEN_ROD, KHAKI, YELLOW, LIGHT_CYAN, LIGHT_SKY_BLUE, THISTLE, ANTIQUE_WHITE, BEIGE, BISQUE,
			                            BLANCHED_ALMOND, WHEAT, CORN_SILK, LEMON_CHIFFON, LIGHT_GOLDEN_ROD_YELLOW, LIGHT_YELLOW, MOCCASIN, NAVAJO_WHITE,
			                            PEACH_PUFF, MISTY_ROSE, LAVENDER_BLUSH, LINEN, OLD_LACE, PAPAYA_WHIP, SEA_SHELL, MINT_CREAM, LAVENDER,
			                            FLORAL_WHITE, ALICE_BLUE, GHOST_WHITE, HONEYDEW, IVORY, AZURE, SNOW, WHITE_SMOKE, CANVAS_WHITE, WHITE};
	...
	public static MyColor getRandomColor() {
		...
		while (isBlackListColor(randomBlackList, mycolors[num]) == true) {
			num = (int) (Math.random() * 138);
		}
		return mycolors[num];
	}
	
	public static boolean isBlackListColor(MyColor[] usedArray, MyColor color) {
		for (MyColor s : usedArray) { if (s == color) return true; }
		return false;
	}
	
	public Color getJavaFXColor() {	// Convert to a color format that GraphicsContext can use
